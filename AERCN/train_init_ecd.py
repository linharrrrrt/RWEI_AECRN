...
parser.add_argument('--learningrate', '-lr', type=float, default=0.0001, 
	help='learning rate')
parser.add_argument('--resume', '-rs',  default=None, help='resume model')
...
trainset = CamLocDataset_ecd("./datasets/" + opt.scene + "/train", mode=min(opt.mode, 1), sparse=opt.sparse, augment=True)
...
if opt.resume is not None:
    ...
    checkpoint = torch.load(opt.resume)# 训练的
...
for epoch in range(epochs):	
	...
	# for image, gt_pose, gt_coords, focal_length, file, mask in trainset_loader:
	for image, gt_pose, gt_coords, focal_length, file in trainset_loader:
		...
		optimizer.zero_grad()
		if iteration%1000==0:
			print('Iteration: %6d, Loss: %.1f, Valid: %.1f%%, Time: %.4fs' % (iteration, loss, num_valid_sc*100, time.time()-start_time), flush=True)
